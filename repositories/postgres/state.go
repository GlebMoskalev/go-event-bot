...
func (p *postgres) GetState(ctx context.Context, chatID int64) (models.State, error) {
	log := logger.SetupLogger(p.log, "repository_state", "GetState", "chat_id", chatID)
...
func (p *postgres) GetStateAndData(ctx context.Context, chatID int64) (models.State, []byte, error) {
	log := logger.SetupLogger(p.log, "repository_state", "GetStateAndData", "chat_id", chatID)
...
`
	...
	log.Info("state and data retrieved successfully", "state", state)
	return state, data, nil
}

func (p *postgres) SetState(ctx context.Context, chatID int64, state models.State, dataJSON []byte) error {
	log := logger.SetupLogger(p.log, "repository_state", "SetState", "chat_id", chatID, "state", state)
...
`
	result, err := p.dbBot.ExecContext(ctx, query, chatID, state, dataJSON)
	if err != nil {
	...
	log.Info("state set successfully", "rows_affected", rowsAffected)
	return nil
}

func (p *postgres) RemoveState(ctx context.Context, chatID int64) error {
	log := logger.SetupLogger(p.log, "repository_state", "RemoveState", "chat_id", chatID)
...
`
	...
	log.Info("state removed successfully", "rows_affected", rowsAffected)
	return nil
}
